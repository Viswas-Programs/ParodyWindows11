...
def main(FILESYSTEM: ParWFS, *args):
    try:
        ...
        files.add_command(label="Open With", command=lambda: fileRouters.openWithSettings(INSTANCES[args[-1]], str(os.path.join(filepath, fileView.item(fileView.focus(), 'values')[0])).replace("\\", "/"), args[-2], args[0] ))
